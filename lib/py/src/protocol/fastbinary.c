...
static int
output_val(PyObject* output, PyObject* value, TType type, PyObject* typeargs, bool utf8str) {
  /*
  ...
  case T_SET: {
    ...
    while ((item = PyIter_Next(iterator))) {
      if (!output_val(output, item, parsedargs.element_type, parsedargs.typeargs, utf8str)) {
        Py_DECREF(item);
        Py_DECREF(iterator);
        return false;
      }
  ...
  case T_MAP: {
    ...
    while (PyDict_Next(value, &pos, &k, &v)) {
      ...
      if (!output_val(output, k, parsedargs.ktag, parsedargs.ktypeargs, utf8str)
          || !output_val(output, v, parsedargs.vtag, parsedargs.vtypeargs, utf8str)) {
        Py_DECREF(k);
        Py_DECREF(v);
        return false;
      }
  ...
  case T_STRUCT: {
    ...
    for (i = 0; i < nspec; i++) {
      ...
      if (!output_val(output, instval, parsedspec.type, parsedspec.typeargs, utf8str)) {
        Py_DECREF(instval);
        return false;
      }
...
static bool
decode_struct(DecodeBuffer* input, PyObject* output, PyObject* spec_seq, bool utf8str) {
...
decode_val(DecodeBuffer* input, TType type, PyObject* typeargs, bool utf8str) {
  ...
  case T_SET: {
    ...
    for (i = 0; i < len; i++) {
      PyObject* item = decode_val(input, parsedargs.element_type, parsedargs.typeargs, utf8str);
      if (!item) {
        Py_DECREF(ret);
        return NULL;
      }
